...
STOP_DISTANCE = 0.3
LIDAR_ERROR = 0.05
LIDAR_AVOID_DISTANCE = 0.4
SAFE_STOP_DISTANCE = STOP_DISTANCE + LIDAR_ERROR
RIGHT_SIDE_INDEX = 270
RIGHT_FRONT_INDEX = 210
LEFT_FRONT_INDEX = 150
LEFT_SIDE_INDEX = 90
WALL_FOLLOW_DISTANCE = SAFE_STOP_DISTANCE + 0.32 #how far should the bot be from the wall
...
class RightWallFollow(Node):
    ...
    def timer_callback(self):
        ...
        if self.stall_time > self.stall_time_threshold:
            ...
            self.cmd.linear.x = 0.08 #stop moving
        ...
        # Is there is an obstacle in front?
            ...
            self.cmd.linear.x = 0.25  # slow down moving forward
            #can follow the right wall?
            if right_lidar_min > WALL_FOLLOW_DISTANCE and self.following_wall:
                    self.cmd.linear.x = 0.0
                    self.cmd.angular.z = -0.55  # currently close to wall, turn right to avoid the front obstacle
                    self.following_wall_time = curr_time # currently following wall
            else:
                self.cmd.linear.x = 0.08
                self.cmd.angular.z = 0.7 #currently not following right wall, turn left to avoid obstacle
            self.publisher_.publish(self.cmd)
            self.following_wall = False
            self.get_logger().info(f'Object in front, turning away: {self.following_wall}')
            ...
            if right_lidar_min < SAFE_STOP_DISTANCE and self.first_wall:   # Too close to the right wall, turn left
                self.cmd.linear.x = 0.08
                self.cmd.angular.z = 0.65 # Turn left
                ...
                self.cmd.linear.x = 0.08
                self.cmd.angular.z = -0.65 # Turn right
                self.get_logger().info(f'Finding right wall, turning right: {right_lidar_min}')
